...
def acl_model(vlan):
    ...
    def ace(grant, protocol, source, destination):
        entries.append({'sequence': (len(entries) + 1) * 10, 'grant': grant,
...
def outputs():
    ...
    for path in sorted((ROOT / 'topologies').glob('*/intent.json')):
        ...
        for device in topology['devices']:
            models = [acl_model(v) for v in topology['vlans']
                      if topology['segmented'] and device['core'] and v['id'] not in (99, 999)]
            acls = [{'name': acl['name'], 'lines': [
                f"{a['sequence']} {a['grant']} {a['protocol']} {endpoint(a['source'])} {endpoint(a['destination'])}"
                for a in acl['aces']]} for acl in models]
